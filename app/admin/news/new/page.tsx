...
export default function NewNewsPage() {
  ...
  useEffect(() => {
    ...
    // Vietnamese to Latin conversion mapping
  ...
  // Generate meta description from excerpt or content (only if not manually edited)
  // ✅ Generate meta description from excerpt or content (only once if empty)
  useEffect(() => {
    if (post.excerpt) {
      setPost(prev => ({ ...prev, metaDescription: post.excerpt }))
    } else if (post.content && !post.metaDescription) {
      // Generate meta description from content if excerpt is empty
      const metaDesc = generateMetaDescription(post.content)
      setPost(prev => ({ ...prev, metaDescription: metaDesc }))
    }else {
      setPost(prev => ({ ...prev, metaDescription: "" })) // reset khi không còn excerpt và content
    }
